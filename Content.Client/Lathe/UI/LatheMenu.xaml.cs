...
public sealed partial class LatheMenu : DefaultWindow
{
    ...
    public void PopulateQueueList(List<LatheQueueEntry> queue)
    {
        ...
        {
            // One row per run of the same recipe, not per job. Every row builds an EntityPrototypeView, and
            // the whole list is thrown away and rebuilt on each state update, so a bulk order queued at an
            // ore processor would otherwise mean hundreds of views rebuilt twice a second.
            if (!_prototypeManager.TryIndex(entry.Recipe, out var recipe))
                continue;
            ...
            queuedRecipeLabel.Text = entry.Count > 1
                ? Loc.GetString("lathe-menu-queue-item-count", ("index", idx), ("name", name), ("count", entry.Count))
                : $"{idx}. {name}";
