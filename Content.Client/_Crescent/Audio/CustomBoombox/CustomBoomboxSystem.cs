...
namespace Content.Client._Crescent.Audio.CustomBoombox;
...
public sealed class CustomBoomboxSystem : SharedCustomBoomboxSystem
{
    ...
    public override void Initialize()
    {
        ...
        _netManager.RegisterNetMessage<MsgCustomBoomboxUpload>();

        SubscribeLocalEvent<CustomBoomboxComponent, AppearanceChangeEvent>(OnAppearanceChange);
        SubscribeLocalEvent<CustomBoomboxComponent, AnimationCompletedEvent>(OnAnimationCompleted);
        SubscribeLocalEvent<CustomBoomboxComponent, AfterAutoHandleStateEvent>(OnAfterState);
    }

    private void OnAfterState(Entity<CustomBoomboxComponent> ent, ref AfterAutoHandleStateEvent args)
    {
        if (!_uiSystem.TryGetOpenUi<CustomBoomboxBoundUserInterface>(ent.Owner, CustomBoomboxUiKey.Key, out var bui))
            return;

        bui.Reload();
    }

    private void OnAnimationCompleted(EntityUid uid, CustomBoomboxComponent component, AnimationCompletedEvent args)
    {
    ...
    private void OnAppearanceChange(EntityUid uid, CustomBoomboxComponent component, ref AppearanceChangeEvent args)
    {
        if (args.Sprite == null)
            return;
    ...
    private void UpdateAppearance(EntityUid uid, JukeboxVisualState visualState, CustomBoomboxComponent component, SpriteComponent sprite)
    {
